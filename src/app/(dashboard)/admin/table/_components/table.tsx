...
export default function TableManagement() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (currentSearch) {
        query.or(`name.ilike.%${currentSearch}%,capacity.ilike.%${currentSearch}%,status.ilike.%${currentSearch}%`);
      }
    ...
    <div className="w-full">
      <div className="flex flex-col lg:flex-row mb-4 gap-2 justify-between w-full">
        ...
        <div className="flex gap-2">
          <Input placeholder="Search by name, capacity and status" onChange={(e) => handleChangeSearch(e.target.value)} />
          <Dialog>

...
export default function DialogCreateTable({ refetch }: { refetch: () => void }) {
  ...
  const [createTableState, createTableAction, isPendingCreateTable] = useActionState(createTable, INITIAL_STATE_TABLE);
  ...
  return <FormTable form={form} onSubmit={onSubmit} isLoading={isPendingCreateTable} type="Create" />;
}

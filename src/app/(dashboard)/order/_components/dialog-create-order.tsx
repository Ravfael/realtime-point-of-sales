...
export default function DialogCreateOrder({ refetch, tables }: { refetch: () => void; tables: Table[] | undefined | null }) {
  ...
  const [createOrderState, createOrderAction, isPendingCreateOrder] = useActionState(createOrder, INITIAL_STATE_ORDER);
      ...
      <Form {...form}>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-4 max-h-[50vh] px-1 overflow-y-auto">
            ...
            />
            <FormSelect form={form} name="status" label="Status" selectItem={STATUS_CREATE_ORDER} />
          </div>
          <DialogFooter>
            <DialogClose asChild>

...
export default function AddOrderItem({ id }: { id: string }) {
  ...
  const { currentSearch, currentFilter, handleChangeSearch, handleChangeFilter } = useDataTable();

  const { data: menus, isLoading: isLoadingMenu } = useQuery({
    queryKey: ["menus", currentSearch, currentFilter],
    queryFn: async () => {
      const query = supabase.from("menus").select("*", { count: "exact" }).order("created_at").eq("is_available", true).ilike("name", `%${currentSearch}%`);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 w-full">
      <div className="space-y-4 lg:w-2/3">
          ...
          <div className="flex flex-col lg:flex-row items-center gap-4">
            ...
            <div className="flex gap-2">
              {FILTER_MENU.map((item) => (
                <Button key={item.value} onClick={() => handleChangeFilter(item.value)} variant={currentFilter === item.value ? "default" : "outline"}>
                  {item.label}
                </Button>
              ))}
            </div>
        ...
        {!isLoadingMenu && menus?.data?.length === 0 && <div className="text-center w-full">Menu not found</div>}
      </div>
      <div className="lg:w-1/3">
        <CartSection order={order} carts={carts} setCarts={setCarts} onAddToCart={handleAddToCart} isLoading={isPendingAddOrderItem} onOrder={handleOrder} />
      </div>
    </div>
  );
}

    ...
    <Card className="w-full shadow-sm">
      <CardContent className="space-y-4">
        ...
        <div className="space-y-4">
            ...
            carts?.map((item: Cart) => (
              <div key={item.menu.id} className="space-y-2">
                ...
                <div className="flex items-center gap-4 w-full">
                  <Input placeholder="Add Note" className="w-full" onChange={(e) => debounce(() => handleAddNote(item.menu!.id, e.target.value), 500)} />
          ...
          <Button onClick={() => onOrder()} className="w-full font-semibold bg-teal-500 hover:bg-teal-600 cursor-pointer text-white">
